$(function () {
    ...
    // open connection
    var connection = new WebSocket('ws://23.21.74.211:14484');

    connection.onopen = function () {
        // first we want users to enter their names
        input.removeAttr('disabled');
        status.text('Enter Password:');
    };
    ...
    connection.onmessage = function (message) {
        ...
        if (json.type === 'login') { // first response from the server with user's color
            if (json.status == 'success')
            {
              ...
              status.text('Admin: ').css('color', 'black');
              input.removeAttr('disabled').focus();
            }else {
              console.log("login failure");
              input.removeAttr('disabled').focus();
              status.text('Incorrect Password').css('color', 'red');
            }
